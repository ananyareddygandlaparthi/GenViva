...
The final score takes into consideration:
- **Visual**: Facial Expressions 
- **Audio**: Words per Minute (WPM), Long Pauses, Filler Words, Clarity
- **Content**: Presentation-Slides Similarity, Slides Coverage
- **Viva Answers**: Correctness, Relevance, Clarity, Depth, Similarity to Audio Transcript and Slides

Viva questions are generated by assessing weaknesses and detecting topics present in slides that are not covered in the presentation and are then evaluated by checking for general correctness and correctness in relation to the slides by using the FAISS index built from the slides and presentation transcript.
