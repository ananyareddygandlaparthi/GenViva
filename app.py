...
QUESTION_TEMPLATES = {
    "general": [
        "What problem does your project solve, and why is it important?",
        "Can you explain your system architecture end to end?",
        "Why did you choose this methodology or model over other alternatives?",
        "How do you evaluate your project, and what metrics do you use?",
        "What are the main limitations of your current implementation?",
    ],
    "nervousness": [
        "Your delivery seemed slightly hesitant at a few points. Can you confidently justify the main design choices in your project?",
        "Suppose the reviewer challenges your core idea. How would you defend it in two or three strong points?",
    ],
    "fluency": [
        "Can you explain your project again in a clear step-by-step flow without skipping the module transitions?",
        "Can you summarize your project in under one minute, covering problem, method, and outcome?",
    ],
    "content_depth": [
        "You briefly touched on some parts of the project. Can you explain the implementation details of the most important module?",
        "What exactly happens internally when your system receives input and produces output?",
    ],
    "technical_understanding": [
        "Can you justify the design or model choices you made, including why you did not choose some common alternatives?",
        "If a reviewer asks about failure cases, edge cases, or limitations, how would you answer?",
    ],
    "retrieval_gap": [
        "You mentioned some concepts only briefly. Can you explain the slide content around your methodology and evaluation in more technical depth?",
        "Which slide in your presentation is the most important for understanding your project, and why?",
    ],
}
...
def extract_keywords(text, max_keywords=80):
    ...
    stop = {
        "the", "and", "is", "are", "a", "an", "to", "of", "in", "for", "on", "with", "by",
        "this", "that", "it", "as", "we", "our", "be", "from", "at", "or", "using", "used",
        "can", "was", "were", "has", "have", "had", "will", "into", "their", "there", "these",
    }
...
def generate_viva_questions(project_text, weak_areas, embedder, index, chunks):
    questions = []

    # 1) general questions
    for q in QUESTION_TEMPLATES["general"][:3]:
        ctx = retrieve_context(q, embedder, index, chunks, k=2)
        questions.append({
            "type": "general",
            "question": q,
            "context": ctx,
    ...
    # 2) weakness-targeted questions
    for area in weak_areas[:3]:
        templates = QUESTION_TEMPLATES.get(area, [])
        if templates:
            q = templates[0]
            ctx = retrieve_context(q, embedder, index, chunks, k=2)
            questions.append({
...
# ANSWER EVALUATION
# =========================================================
def evaluate_answer(answer, question_obj, embedder):
    ...
    context_text = " ".join([c["text"] for c in question_obj.get("context", [])]).strip()
    if not context_text:
        context_text = question_obj["question"]

    emb = embedder.encode([answer, context_text], convert_to_tensor=True, normalize_embeddings=True)
    sim = float(util.cos_sim(emb[0], emb[1]).item())
    relevance = clamp(((sim + 1) / 2) * 100.0)

    words = re.findall(r"\b\w+\b", answer)
    length = len(words)
    depth = clamp(min(length * 1.5, 100.0))

    sentence_count = max(1, len(re.findall(r"[.!?]", answer)) or 1)
    avg_sentence_len = length / sentence_count
    clarity = clamp(100.0 - abs(avg_sentence_len - 16.0) * 3.0)

    score = clamp((0.5 * relevance) + (0.3 * depth) + (0.2 * clarity))

    if score >= 75:
        feedback = "Strong and correct answer. It accurately reflects the project methodology from your slides."
    elif score >= 55:
        feedback = "Decent answer, but it could be more accurate or closer to the technical depth of your slides."
    else:
        feedback = "Weak/Incorrect answer. The content does not align well with the core concepts found in your slides."
    ...
    if st.session_state.phase == 1:
        ...
        if st.button("Evaluate Viva Answers"):
            ...
            for q, ans in zip(questions, answers):
                evaluation = evaluate_answer(ans, q, embedder)
